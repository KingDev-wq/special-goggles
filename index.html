...
async function enviar() {
  ...
  if (!discordTag) { statusEl.textContent='⚠️ Digite seu @ do Discord'; statusEl.style.color='yellow'; return; }
  ...
  try {
    ...
    if(resultado.success){ statusEl.textContent='✅ Verificação enviada com sucesso!'; statusEl.style.color='lightgreen'; }
    else { statusEl.textContent='❌ Erro ao enviar!'; statusEl.style.color='red'; }
  } catch(err){
    console.error(err);
  ...
  // POST para enviar @ e IP para o webhook
  if (req.method === "POST") {
    const { discordTag } = req.body;
    if (!discordTag) return res.status(400).json({error:"Campo @ do Discord obrigatório"});
